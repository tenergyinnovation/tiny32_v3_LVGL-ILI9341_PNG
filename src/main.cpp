...
// #define MAX_IMAGE_WIDTH 160 // Adjust for your images
...
void loop()
{
  ...
  int16_t rc = png.openFLASH((uint8_t *)cmu, sizeof(cmu), pngDraw);
  // int16_t rc = png.openFLASH((uint8_t *)tenergy, sizeof(tenergy), pngDraw);
  // int16_t rc = png.openFLASH((uint8_t *)EagleEye, sizeof(EagleEye), pngDraw);
  if (rc == PNG_SUCCESS) {
